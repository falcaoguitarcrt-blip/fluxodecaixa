...
describe("investment portfolio summaries", () => {
  it("keeps monthly contributions separate from accumulated market value", () => {
    expect(summarizeInvestments([{ investedAmount: "500.00", marketValue: "550.00" }], [{ investedAmount: "500.00", marketValue: "550.00" }, { investedAmount: "1000.00", marketValue: "1200.00" }])).toEqual({ monthlyContribution: 500, investedAmount: 1500, marketValue: 1750, investmentResult: 250 });
  });

  it("keeps accumulated patrimony when the selected month has no new contribution", () => {
    expect(summarizeInvestments([], [{ investedAmount: "1000.00", marketValue: "1180.00" }])).toEqual({ monthlyContribution: 0, investedAmount: 1000, marketValue: 1180, investmentResult: 180 });
  });

  it("returns zero for an empty selected month and empty portfolio", () => {
    expect(summarizeInvestments([], [])).toEqual({ monthlyContribution: 0, investedAmount: 0, marketValue: 0, investmentResult: 0 });
  });
});
...
describe("couple dashboard aggregation", () => {
  it("consolidates profile totals, cards and institutions without static fallback values", () => {
    const result = buildCoupleDashboard([
      { profileKey: "felipe", displayName: "Felipe", profileId: 1, summary: { income: 3000, expenses: 1000, balance: 2000, invested: 4000, investedAmount: 3500, monthlyContribution: 3500, billsPending: 100, cardInstallments: 50, cardTotal: 100 }, cards: [{ id: 10, name: "Inter Felipe", brand: "Visa", dueDay: 20, closingDay: 13 }], purchases: [{ cardId: 10, totalAmount: "100.00", installmentAmount: "50.00" }], investments: [{ institution: "Inter", investedAmount: "3500.00", marketValue: "4000.00" }], bills: [{}] },
      { profileKey: "sara", displayName: "Sara", profileId: 2, summary: { income: 1000, expenses: 200, balance: 800, invested: 1500, investedAmount: 1500, monthlyContribution: 1500, billsPending: 50, cardInstallments: 25, cardTotal: 40 }, cards: [{ id: 11, name: "Nubank Sara", brand: "Mastercard", dueDay: 10, closingDay: 3 }], purchases: [{ cardId: 11, totalAmount: "40.00", installmentAmount: "25.00" }], investments: [{ institution: "Inter", investedAmount: "1500.00", marketValue: "1500.00" }], bills: [{}, {}] },
    ]);
    expect(result.summary).toMatchObject({ income: 4000, expenses: 1200, balance: 2800, invested: 5500, investedAmount: 5000, monthlyContribution: 5000, investmentResult: 500, billsPending: 150, cardInstallments: 75, cardTotal: 140, commitment: 35.63, totalCards: 2, totalBills: 3 });
  ...
  it("filters the consolidated snapshot by authenticated user and keeps only Felipe/Sara profiles", () => {
    const sharedPart = { profileKey: "felipe" as const, displayName: "Felipe", profileId: 1, userId: 7, summary: { income: 100, expenses: 25, balance: 75, invested: 10, investedAmount: 8, monthlyContribution: 8, billsPending: 0, cardInstallments: 0, cardTotal: 0 }, cards: [], purchases: [], investments: [], bills: [] };
  ...
  it("returns an honest empty consolidated state and validates the month contract", async () => {
    ...
    expect(empty.summary).toMatchObject({ income: 0, expenses: 0, balance: 0, invested: 0, monthlyContribution: 0, investmentResult: 0, commitment: 0, totalCards: 0, totalBills: 0 });
    const caller = appRouter.createCaller(createContext());
    ...
    expect(response.summary).toEqual(expect.objectContaining({ income: expect.any(Number), expenses: expect.any(Number), balance: expect.any(Number), invested: expect.any(Number), investedAmount: expect.any(Number), monthlyContribution: expect.any(Number), investmentResult: expect.any(Number) }));
