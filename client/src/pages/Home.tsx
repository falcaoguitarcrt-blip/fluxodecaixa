...
function LiveTransactionsCard({ onAdd, onEdit, profileId }: { onAdd: () => void; onEdit?: (item: any) => void; profileId?: number }) {
...
function Overview({ onAdd, onEdit, profileId, month = "2026-08", summary }: { onAdd: () => void; onEdit: (item: any) => void; profileId?: number; month?: string; summary?: { balance: number; income: number; expenses: number; invested: number; commitment: number; series?: Array<{ date: string; income: number; expenses: number }> } }) {
  return <div className="space-y-5"><div className="stats-grid"><StatCard label="Saldo do mês" value={formatMoney(summary?.balance ?? 597.88)} detail="entradas menos saídas do período" tone="blue" icon={CircleDollarSign} /><StatCard label="Total de entradas" value={formatMoney(summary?.income ?? 8240)} detail="receitas registradas no mês" tone="mint" icon={ArrowDownLeft} /><StatCard label="Total de saídas" value={formatMoney(summary?.expenses ?? 7642.12)} detail="despesas registradas no mês" tone="clay" icon={ArrowUpRight} /><StatCard label="Comprometido" value={`${(summary?.commitment ?? 79.9).toFixed(1)}%`} detail="atenção quando passa de 70%" tone="gold" icon={TrendingUp} /></div><div className="content-grid"><ChartCard series={summary?.series} /><CommitmentCard commitment={summary?.commitment} /><DaySummary /><RoutineCard profileId={profileId} month={month} /><aside className="assistant-card"><div className="assistant-orb"><img src={ORB_URL} alt="" /></div><div><p className="eyebrow">sarinha ia</p><h2>Um olhar para o seu mês</h2><p className="mt-2 text-sm leading-6 text-slate-400">Você está mantendo 18% da receita em investimentos. Quer observar esse ritmo juntos?</p></div><button onClick={() => toast.success("Sarinha IA: análise adicionada ao seu resumo") } className="assistant-action">Conversar com a Sarinha <ChevronRight className="h-4 w-4" /></button></aside><WordDayCard /></div><LiveTransactionsCard onAdd={onAdd} onEdit={onEdit} profileId={profileId} /><GovernanceCard profileId={profileId} /></div>;
}
...
function Accounts({ profileId, month = "2026-08", onAdd, onEdit }: { profileId?: number; month?: string; onAdd: () => void; onEdit: (item: any) => void }) {
  const { isAuthenticated } = useAuth();
  const { data } = trpc.finance.bootstrap.useQuery({ profileId, month }, { enabled: isAuthenticated && !!profileId });
  ...
  const visibleBills = bills.filter((bill) => (!statusFilter || resolveBillStatus(bill.status, bill.dueDate) === statusFilter) && (!effectiveMonth || new Date(bill.dueDate).toISOString().slice(0, 7) === effectiveMonth));
...
function Investments({ profileId, month = "2026-08", person, onAdd, onEdit }: { profileId?: number; month?: string; person?: Person; onAdd: () => void; onEdit: (item: any) => void }) {
  const { isAuthenticated } = useAuth();
  const { data } = trpc.finance.bootstrap.useQuery({ profileId, month }, { enabled: isAuthenticated && !!profileId });
  const [monthFilter, setMonthFilter] = useState("");
  ...
  const effectiveMonth = monthFilter || month;
  const rows = allRows.filter((item) => (!effectiveMonth || new Date(item.investedAt).toISOString().slice(0, 7) === effectiveMonth) && (!bankFilter || item.institution === bankFilter) && (!categoryFilter || item.category === categoryFilter));
...
function Cards({ profileId, month = "2026-08", onAddCard, onAddPurchase, onEditCard, onEditPurchase }: { profileId?: number; month?: string; onAddCard: () => void; onAddPurchase: () => void; onEditCard: (item: any) => void; onEditPurchase: (item: any) => void }) {
  ...
  const visiblePurchases = purchaseRows.filter((item) => (!activeCardMonth || new Date(item.purchaseDate).toISOString().slice(0, 7) === activeCardMonth) && (!selectedCardId || item.cardId === selectedCardId) && (!categoryFilter || item.category === categoryFilter) && (!onlyInstallments || item.installments > 1));
  ...
  return <div className="space-y-5"><SectionHeader eyebrow="controle separado · agosto de 2026" title="Cartão de crédito" actions={<><ActionButton variant="ghost" onClick={() => { const selected = cards.find((card) => card.id === selectedCardId) ?? cards[0]; if (selected) onEditCard(selected); else toast.info("Cadastre um cartão antes de editar"); }}><Pencil className="h-4 w-4" /> editar cartão</ActionButton><ActionButton variant="ghost" onClick={onAddCard}><Plus className="h-4 w-4" /> novo cartão</ActionButton><ActionButton onClick={onAddPurchase}><Plus className="h-4 w-4" /> cadastrar e salvar fatura</ActionButton></>} /><FilterBar type="cards" cardOptions={cards} categoryOptions={cardCategories} values={{ month: cardMonth, cardId: selectedCardId?.toString(), category: categoryFilter }} onChange={(next) => { setCardMonth(next.month ?? ""); setSelectedCardId(next.cardId ? Number(next.cardId) : undefined); setCategoryFilter(next.category ?? ""); }} /><div className="segmented"><span>Mostrar compras por</span><button className={!showTotal ? "active" : ""} onClick={() => setShowTotal(false)}>Parcela do mês</button><button className={showTotal ? "active" : ""} onClick={() => setShowTotal(true)}>Valor total</button></div><div className="segmented"><span>Exibir</span><button className={!onlyInstallments ? "active" : ""} onClick={() => setOnlyInstallments(false)}>Todas</button><button className={onlyInstallments ? "active" : ""} onClick={() => setOnlyInstallments(true)}>Só parceladas do mês</button></div><ActionButton onClick={() => { const csv = serializeCardPurchasesCsv(visiblePurchases, cards); const blob = new Blob([csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = "fluxo-faturas.csv"; link.click(); URL.revokeObjectURL(url); toast.success("CSV de faturas exportado"); }}><Download className="h-4 w-4" /> exportar CSV</ActionButton><article className="panel card-statement"><div className="statement-head"><span className="checkbox" /><span>Cartão <span className="sort-badge">↕</span></span><span>Mês <span className="sort-badge">↕</span></span><span>Vencimento</span><span className="text-right">Valor</span></div>{cards.length ? cards.map((card) => { const cardPurchases = visiblePurchases.filter((item) => item.cardId === card.id); const cardValue = cardPurchases.reduce((sum, item) => sum + Number(showTotal ? item.totalAmount : item.installmentAmount), 0); return <button className={`statement-row text-left ${selectedCardId === card.id ? "selected-row" : ""}`} key={card.id} onClick={() => setSelectedCardId(selectedCardId === card.id ? undefined : card.id)}><span className="checkbox" /><span>{card.name}</span><span>agosto de 2026</span><span>{String(card.dueDay).padStart(2, "0")}/09/2026</span><strong className="text-right">{formatMoney(cardValue)}</strong></button>; }) : <div className="empty-state"><h2>Nenhum cartão cadastrado.</h2><p>Cadastre um cartão para controlar suas faturas.</p></div>}<div className="statement-total"><div><p className="eyebrow">fechamento dia 13 · vencimento dia 20</p><h2>Fatura de agosto</h2><p className="mt-2 text-sm text-slate-400">{visiblePurchases.length} compras internas · em aberto</p></div><strong>{formatMoney(showTotal ? (selectedCardId ? visibleTotal : cardTotal) : (selectedCardId ? visibleInstallments : cardInstallments))}</strong><button className="inline-link" onClick={() => toast.info("Detalhes da fatura")}>ver detalhes <ChevronRight className="h-4 w-4" /></button></div></article><div className="card-detail-grid">{cards.map((card) => { const cardPurchases = visiblePurchases.filter((item) => item.cardId === card.id); const amount = cardPurchases.reduce((sum, item) => sum + Number(showTotal ? item.totalAmount : item.installmentAmount), 0); return <article className="card-detail" key={`${card.id}-detail`}><div className="card-detail-header"><span className="card-brand">{card.brand.slice(0, 3)}</span><div><h3>{card.name}</h3><p className="card-detail-meta">fecha dia {card.closingDay} · vence dia {card.dueDay}</p></div></div><p className="card-detail-total">{formatMoney(amount)}</p><p className="card-detail-meta">{cardPurchases.length} compras internas · em aberto</p><div className="card-detail-footer"><span>Fatura do mês</span><button className="edit-link" onClick={() => { if (cardPurchases[0]) onEditPurchase(cardPurchases[0]); else toast.info("Não há compras nesta fatura para editar"); }}>editar compra</button><strong>Ver detalhes <ChevronRight className="inline h-3 w-3" /></strong></div></article>; })}</div></div>;
}

function Couple({ month = "2026-08" }: { month?: string }) {
  const { isAuthenticated } = useAuth();
  const { data: profiles = [] } = trpc.finance.profiles.useQuery(undefined, { enabled: isAuthenticated });
  const felipeId = profiles.find((profile) => profile.profileKey === "felipe")?.id;
  const saraId = profiles.find((profile) => profile.profileKey === "sara")?.id;
  const { data: felipeData } = trpc.finance.bootstrap.useQuery(felipeId ? { profileId: felipeId, month } : undefined, { enabled: isAuthenticated && !!felipeId });
  const { data: saraData } = trpc.finance.bootstrap.useQuery(saraId ? { profileId: saraId, month } : undefined, { enabled: isAuthenticated && !!saraId });
  const felipe = felipeData?.summary;
  const sara = saraData?.summary;
  const income = (felipe?.income ?? 0) + (sara?.income ?? 0);
  const expenses = (felipe?.expenses ?? 0) + (sara?.expenses ?? 0);
  const balance = (felipe?.balance ?? 0) + (sara?.balance ?? 0);
  const invested = (felipe?.invested ?? 0) + (sara?.invested ?? 0);
  const commitment = income > 0 ? ((expenses + (felipe?.billsPending ?? 0) + (sara?.billsPending ?? 0) + (felipe?.cardInstallments ?? 0) + (sara?.cardInstallments ?? 0)) / income) * 100 : 86.5;
  const hasData = Boolean(felipeData || saraData);
  const displayBalance = hasData ? balance : 597.88;
  const displayIncome = hasData ? income : 4443.57;
  const displayExpenses = hasData ? expenses : 3845.69;
  const displayInvested = hasData ? invested : 24460.22;
  const totalCards = (felipeData?.cards.length ?? 0) + (saraData?.cards.length ?? 0);
  const totalBills = (felipeData?.bills.length ?? 0) + (saraData?.bills.length ?? 0);
  const invoiceInstallments = (felipe?.cardInstallments ?? 0) + (sara?.cardInstallments ?? 0);
  const invoiceTotal = (felipe?.cardTotal ?? 0) + (sara?.cardTotal ?? 0);
  const felipeShare = displayBalance > 0 ? Math.round((Math.max(0, felipe?.balance ?? 0) / displayBalance) * 100) : 64;
  const saraShare = Math.max(0, 100 - felipeShare);
  return <div className="space-y-5"><div className="couple-hero"><div className="couple-figures"><UserRound className="h-10 w-10" /><UsersRound className="h-12 w-12 -ml-3" /></div><div><p className="eyebrow">visão consolidada · agosto de 2026</p><h2>Resumo geral do casal</h2><p className="mt-2 text-sm text-slate-400">Felipe + Sara em uma única leitura visual, com compromissos e patrimônio lado a lado.</p></div></div><div className="stats-grid four"><StatCard label="Saldo consolidado" value={formatMoney(displayBalance)} detail="contas e pagamentos do mês" tone="blue" /><StatCard label="Entradas" value={formatMoney(displayIncome)} detail="receitas de Felipe + Sara" tone="mint" /><StatCard label="Saídas" value={formatMoney(displayExpenses)} detail="contas e cartões" tone="clay" /><StatCard label="Comprometido" value={`${Math.min(100, commitment).toFixed(1)}%`} detail="das entradas do casal" tone="gold" /></div><div className="couple-detail-grid"><article className="panel couple-detail-card"><p className="eyebrow">contas</p><h3>Visão financeira</h3><div className="couple-detail-value">{formatMoney(displayBalance)}</div><p>Saldo disponível · {totalBills} contas cadastradas</p></article><article className="panel couple-detail-card"><p className="eyebrow">investimentos</p><h3>Patrimônio investido</h3><div className="couple-detail-value">{formatMoney(displayInvested)}</div><p>{(felipeData?.investments.length ?? 0) + (saraData?.investments.length ?? 0)} registros de investimento</p></article><article className="panel couple-detail-card"><p className="eyebrow">cartões</p><h3>Faturas do mês</h3><div className="couple-detail-value">{formatMoney(hasData ? invoiceInstallments : 0)}</div><p>{totalCards} cartões · {formatMoney(hasData ? invoiceTotal : 0)} em compras totais</p></article></div><div className="content-grid couple-grid"><article className="panel person-panel"><div className="person-head"><div className="avatar avatar-blue">F</div><div><p className="eyebrow">fluxo felipe</p><h2>Visão individual</h2></div><ChevronRight className="ml-auto h-5 w-5 text-slate-500" /></div><p className="person-value">{formatMoney(hasData ? felipe?.balance ?? 0 : 597.88)}</p><div className="progress-line"><span style={{ width: `${felipeShare}%` }} /></div><p className="mt-3 text-xs text-slate-500">{felipeShare}% do saldo consolidado</p></article><article className="panel person-panel"><div className="person-head"><div className="avatar avatar-purple">S</div><div><p className="eyebrow">fluxo sara</p><h2>Visão individual</h2></div><ChevronRight className="ml-auto h-5 w-5 text-slate-500" /></div><p className="person-value">{formatMoney(hasData ? sara?.balance ?? 0 : 0)}</p><div className="progress-line purple"><span style={{ width: `${saraShare}%` }} /></div><p className="mt-3 text-xs text-slate-500">{saraShare}% do saldo consolidado</p></article></div></div>;
}
...
type FinanceFormKind = "transaction" | "bill" | "investment" | "card" | "purchase";
...
function FinanceFormModal({ kind, item, profileId, cards = [], onClose }: { kind: FinanceFormKind; item?: FinanceFormItem; profileId?: number; cards?: Array<{ id: number; name: string }>; onClose: () => void }) {
  const initial = useMemo(() => ({
    description: String(item?.description ?? ""), category: String(item?.category ?? "Casa"), bank: String(item?.bank ?? "Inter"), amount: String(item?.amount ?? ""), date: inputDate(item?.date, "2026-08-21"), direction: String(item?.direction ?? "out"), responsible: String(item?.responsible ?? "Felipe"), dueDate: inputDate(item?.dueDate, "2026-08-25"), status: String(item?.status ?? "pending"), institution: String(item?.institution ?? "Inter"), investedAmount: String(item?.investedAmount ?? ""), marketValue: String(item?.marketValue ?? ""), investedAt: inputDate(item?.investedAt, "2026-08-21"), name: String(item?.name ?? ""), brand: String(item?.brand ?? "Visa"), dueDay: String(item?.dueDay ?? "20"), closingDay: String(item?.closingDay ?? "13"), cardId: String(item?.cardId ?? cards[0]?.id ?? ""), purchaseDate: inputDate(item?.purchaseDate, "2026-08-21"), totalAmount: String(item?.totalAmount ?? ""), installments: String(item?.installments ?? "1"), currentInstallment: String(item?.currentInstallment ?? "1"), notes: String(item?.notes ?? "")
  }), [item, cards]);
  ...
  const utils = trpc.useUtils();
  const title = item ? "Editar registro" : kind === "transaction" ? "Adicionar movimentação" : kind === "bill" ? "Adicionar conta" : kind === "investment" ? "Adicionar investimento" : kind === "card" ? "Adicionar cartão" : "Adicionar compra à fatura";
  const isPending = createTransaction.isPending || updateTransaction.isPending || createBill.isPending || updateBill.isPending || createInvestment.isPending || updateInvestment.isPending || createCreditCard.isPending || updateCreditCard.isPending || createCardPurchase.isPending || updateCardPurchase.isPending;
  const finish = (message: string) => { utils.finance.bootstrap.invalidate(); utils.finance.transactions.invalidate(); utils.finance.audit.invalidate(); toast.success(message); onClose(); };
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal-card" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="eyebrow">{item ? "edição persistente" : "novo registro persistente"}</p><h2>{title}</h2><p className="mt-2 text-sm text-slate-400">Os dados serão salvos no perfil selecionado.</p></div><button className="modal-close" onClick={onClose} aria-label="Fechar"><X className="h-5 w-5" /></button></div><div className="modal-form">{kind === "transaction" && <>{field("Descrição", "description", "text", "ex.: mercado, salário")}{field("Data", "date", "date")}{field("Categoria", "category", "text", "ex.: Casa")}{field("Banco", "bank", "text", "ex.: Inter")}<div className="kind-toggle"><button className={form.direction === "in" ? "active" : ""} onClick={() => update("direction", "in")}><ArrowDownLeft className="h-4 w-4" /> Entrada</button><button className={form.direction === "out" ? "active" : ""} onClick={() => update("direction", "out")}><ArrowUpRight className="h-4 w-4" /> Saída</button></div>{field("Valor", "amount", "text", "R$ 0,00")}{field("Observações", "notes", "text", "opcional")}</>}{kind === "bill" && <>{field("Descrição", "description", "text", "ex.: aluguel")}{field("Vencimento", "dueDate", "date")}{field("Valor", "amount", "text", "R$ 0,00")}{field("Responsável", "responsible", "text", "Felipe ou Sara")}<label>Status<select value={form.status} onChange={(event) => update("status", event.target.value)}><option value="pending">Pendente</option><option value="paid">Paga</option><option value="late">Atrasada</option></select></label></>}{kind === "investment" && <>{field("Descrição", "description", "text", "ex.: Tesouro Selic")}{field("Categoria", "category", "text", "ex.: Renda fixa")}{field("Instituição", "institution", "text", "ex.: Inter")}{field("Data", "investedAt", "date")}{field("Valor investido", "investedAmount", "text", "R$ 0,00")}{field("Valor de mercado", "marketValue", "text", "R$ 0,00")}</>}{kind === "card" && <>{field("Nome do cartão", "name", "text", "ex.: Nubank")}{field("Bandeira", "brand", "text", "Visa, Mastercard...")}{field("Dia de fechamento", "closingDay", "number", "1 a 31")}{field("Dia de vencimento", "dueDay", "number", "1 a 31")}</>}{kind === "purchase" && <>{<label>Cartão<select value={form.cardId} onChange={(event) => update("cardId", event.target.value)}><option value="">Selecione um cartão</option>{cards.map((card) => <option key={card.id} value={card.id}>{card.name}</option>)}</select></label>}{field("Descrição", "description", "text", "ex.: supermercado")}{field("Categoria", "category", "text", "ex.: Alimentação")}{field("Data da compra", "purchaseDate", "date")}{field("Valor total", "totalAmount", "text", "R$ 0,00")}{field("Número de parcelas", "installments", "number", "1")}{field("Parcela atual", "currentInstallment", "number", "1")}</>}</div><div className="modal-footer"><button className="text-sm text-slate-400 transition hover:text-white" onClick={onClose}>cancelar</button><ActionButton onClick={submit} disabled={isPending}>{isPending ? "salvando..." : <><Check className="h-4 w-4" /> salvar</>}</ActionButton></div></div></div>;
}
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    if (active === "investments") return <Investments profileId={activeProfileId} month={activeMonth} person={person} onAdd={() => openForm("investment")} onEdit={(item) => openForm("investment", item)} />;
