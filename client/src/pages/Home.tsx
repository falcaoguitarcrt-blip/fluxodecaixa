...
function Investments({ profileId, month = "2026-08", person, onAdd, onEdit, onAddGoal, onEditGoal }: { profileId?: number; month?: string; person?: Person; onAdd: () => void; onEdit: (item: any) => void; onAddGoal: () => void; onEditGoal: (item: FinanceFormItem) => void }) {
  const { isAuthenticated } = useAuth();
  const { data } = trpc.finance.bootstrap.useQuery({ profileId, month }, { enabled: isAuthenticated && !!profileId });
  const [monthFilter, setMonthFilter] = useState("");
...
function FinanceFormModal({ kind, item, profileId, cards = [], onClose }: { kind: FinanceFormKind; item?: FinanceFormItem; profileId?: number; cards?: Array<{ id: number; name: string }>; onClose: () => void }) {
  const initial = useMemo(() => ({
    description: String(item?.description ?? ""), category: String(item?.category ?? "Casa"), bank: String(item?.bank ?? "Inter"), amount: String(item?.amount ?? ""), date: inputDate(item?.date, "2026-08-21"), direction: String(item?.direction ?? "out"), responsible: String(item?.responsible ?? "Felipe"), dueDate: inputDate(item?.dueDate, "2026-08-25"), status: String(item?.status ?? "pending"), institution: String(item?.institution ?? "Inter"), investedAmount: String(item?.investedAmount ?? ""), marketValue: String(item?.marketValue ?? ""), investedAt: inputDate(item?.investedAt, "2026-08-21"), name: String(item?.name ?? ""), targetAmount: String(item?.targetAmount ?? ""), currentAmount: String(item?.currentAmount ?? "0"), targetDate: inputDate(item?.targetDate, ""), brand: String(item?.brand ?? "Visa"), dueDay: String(item?.dueDay ?? "20"), closingDay: String(item?.closingDay ?? "13"), cardId: String(item?.cardId ?? cards[0]?.id ?? ""), purchaseDate: inputDate(item?.purchaseDate, "2026-08-21"), totalAmount: String(item?.totalAmount ?? ""), installments: String(item?.installments ?? "1"), currentInstallment: String(item?.currentInstallment ?? "1"), notes: String(item?.notes ?? "")
  }), [item, cards]);
