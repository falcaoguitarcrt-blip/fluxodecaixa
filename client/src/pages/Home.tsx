...
function Cards({ profileId, month = "2026-08", onAddCard, onAddPurchase, onEditCard, onEditPurchase }: { profileId?: number; month?: string; onAddCard: () => void; onAddPurchase: () => void; onEditCard: (item: any) => void; onEditPurchase: (item: any) => void }) {
  ...
  const statementMonthDate = new Date(`${activeCardMonth}-01T12:00:00`);
  const statementMonthLabel = statementMonthDate.toLocaleDateString("pt-BR", { month: "long", year: "numeric" });
  const statementDueDate = (dueDay: number) => { const dueDate = new Date(statementMonthDate); dueDate.setMonth(dueDate.getMonth() + 1); dueDate.setDate(Math.min(dueDay, new Date(dueDate.getFullYear(), dueDate.getMonth() + 1, 0).getDate())); return dueDate.toLocaleDateString("pt-BR"); };
