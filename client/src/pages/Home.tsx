...
function PeriodControl({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center gap-2 ${compact ? "scale-95 origin-left" : ""}`}>
      ...
      <div className="period-select"><CalendarDays className="h-4 w-4 period-control-icon" /><span>agosto de 2026</span><ChevronDown className="h-4 w-4 period-control-icon" /></div>
...
function EmptyState({ icon: Icon, title, description, actionLabel, onAction, className = "" }: { icon: typeof Receipt; title: string; description: string; actionLabel?: string; onAction?: () => void; className?: string }) {
  return <div className={`empty-state ${className}`}><div className="empty-state-icon"><Icon className="h-6 w-6" /></div><h2>{title}</h2><p>{description}</p>{actionLabel && onAction && <ActionButton onClick={onAction}><Plus className="h-4 w-4" /> {actionLabel}</ActionButton>}</div>;
}
function StatCard({ label, value, detail, tone = "blue", icon: Icon, notice, urgent = false }: { label: string; value: string; detail?: string; tone?: string; icon?: typeof TrendingUp; notice?: string; urgent?: boolean }) {
  return (
    <article className={`stat-card stat-${tone}${urgent ? " stat-urgent" : ""}`} data-urgent={urgent ? "true" : undefined}>
      <div className="flex items-center justify-between gap-3">
        <p className="eyebrow">{label}</p>
        {Icon && <Icon className={`h-4 w-4 ${urgent ? "stat-icon-urgent" : "text-slate-500"}`} aria-hidden="true" /> }
      </div>
      <p className="stat-value">{value}</p>
      {detail && <p className="stat-detail">{detail}</p>}
      {notice && <p className="stat-notice"><Info className="h-3.5 w-3.5 shrink-0" />{notice}</p>}
    </article>
  );
}
  ...
  return (
    <article className="panel chart-panel">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="legend"><span><i className="legend-dot chart-income-dot" /> Entradas</span><span><i className="legend-dot chart-expense-dot" /> Saídas</span></div>
      </div>
...
function RoutineCard({ profileId, month = currentMonthKey() }: { profileId?: number; month?: string }) {
  ...
  return <article className="panel routine-panel"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">rotina financeira</p><h2>Próximos compromissos</h2></div><button className="inline-link" onClick={() => toast.info("A central de lembretes será ampliada no próximo marco")}>ver tudo <ChevronRight className="h-4 w-4" /></button></div>{overdueBills.length ? overdueBills.slice(0, 2).map((item) => <div className="routine-row" key={`late-${item.id}`}><div className="icon-tile clay"><Bell className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="font-medium routine-overdue-title">Atrasada: {item.description}</p><p className="mt-1 text-xs text-slate-500">Venceu em {new Date(item.dueDate).toLocaleDateString("pt-BR")}</p></div></div>) : upcomingBills.length ? upcomingBills.slice(0, 2).map((item) => <div className="routine-row" key={`upcoming-${item.id}`}><div className="icon-tile gold"><CalendarDays className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="font-medium text-slate-200">{item.description}</p><p className="mt-1 text-xs text-slate-500">Vence em {new Date(item.dueDate).toLocaleDateString("pt-BR")} · {formatMoney(Number(item.amount))}</p></div></div>) : reminders.length ? reminders.map((item) => <div className="routine-row" key={item.id}><div className="icon-tile gold"><Bell className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="font-medium text-slate-200">{item.title}</p><p className="mt-1 text-xs text-slate-500">{new Date(item.dueDate).toLocaleDateString("pt-BR")} · {item.kind}</p></div></div>) : <div className="empty-state compact"><h2>Nenhum lembrete próximo.</h2><p>As contas importantes aparecerão aqui.</p></div>}{budgets.length ? <div className="routine-budget"><span>Orçamento · {budgets[0].category}</span><strong>{formatMoney(budgets[0].spent)} / {formatMoney(budgets[0].limit)}</strong><div className="progress-line"><span style={{ width: `${budgets[0].percent}%` }} /></div></div> : <div className="routine-budget"><span>Recorrências ativas</span><strong>{data?.recurring?.filter((item) => item.active).length ?? 0}</strong></div>}</article>;
}
...
function FilterBar({ type, values = {}, onChange, cardOptions = [], bankOptions = [], categoryOptions = [] }: { type: "accounts" | "investments" | "cards"; values?: FilterValues; onChange?: (values: FilterValues) => void; cardOptions?: Array<{ id: number; name: string }>; bankOptions?: string[]; categoryOptions?: string[] }) {
...
function Investments({ profileId, month = currentMonthKey(), person, onAdd, onEdit, onAddGoal, onEditGoal }: { profileId?: number; month?: string; person?: Person; onAdd: () => void; onEdit: (item: any) => void; onAddGoal: () => void; onEditGoal: (item: FinanceFormItem) => void }) {
  ...
  const portfolioRows = data?.investmentPortfolio ?? allRows;
  ...
  const portfolioFilteredRows = portfolioRows.filter((item) => (!bankFilter || item.institution === bankFilter) && (!categoryFilter || item.category === categoryFilter));
  const monthlyContribution = rows.reduce((sum, item) => sum + Number(item.investedAmount), 0);
  const portfolioInvested = portfolioFilteredRows.reduce((sum, item) => sum + Number(item.investedAmount), 0);
  const portfolioMarket = portfolioFilteredRows.reduce((sum, item) => sum + Number(item.marketValue), 0);
  const hasUnpricedMarketValue = hasUnpricedInvestments(portfolioFilteredRows);
  return <div className="space-y-5"><SectionHeader eyebrow={`patrimônio do perfil ${person?.toLowerCase() ?? "ativo"}`} title="Investimentos" actions={<><ActionButton variant="ghost" onClick={() => toast.info("Use editar na tabela para alterar um investimento")}><Pencil className="h-4 w-4" /></ActionButton><ActionButton onClick={onAdd}><Plus className="h-4 w-4" /> novo investimento</ActionButton></>} />{isSara && <SavingsGoalsCard profileId={profileId} onAdd={() => onAddGoal()} onEdit={(item) => onEditGoal(item)} />}<FilterBar type="investments" bankOptions={Array.from(new Set([...allRows, ...portfolioRows].map((item) => item.institution)))} categoryOptions={Array.from(new Set([...allRows, ...portfolioRows].map((item) => item.category)))} values={{ month: effectiveMonth, bank: bankFilter, category: categoryFilter }} onChange={(next) => { setMonthFilter(next.month ?? ""); setBankFilter(next.bank ?? ""); setCategoryFilter(next.category ?? ""); }} /><div className="stats-grid four"><StatCard label="Aportes no mês" value={formatMoney(monthlyContribution)} detail={`competência ${formatMonthLabel(effectiveMonth)}`} tone="blue" /><StatCard label="Patrimônio de mercado" value={formatMoney(portfolioMarket)} detail="posição acumulada do perfil" tone="mint" /><StatCard label="Resultado acumulado" value={formatMoney(portfolioFilteredRows.length ? portfolioMarket - portfolioInvested : 0)} detail="mercado menos aportado" tone="gold" notice={hasUnpricedMarketValue ? "Atualize o valor de mercado dos seus ativos para um cálculo de rentabilidade real." : undefined} /><StatCard label="Último aporte" value={portfolioFilteredRows[0] ? new Date(portfolioFilteredRows[0].investedAt).toLocaleDateString("pt-BR") : "sem dados"} detail="data do registro mais recente" tone="clay" /></div><div className="institution-strip"><p className="eyebrow">saldo por instituição</p><div className="flex flex-wrap gap-8">{portfolioFilteredRows.length ? Array.from(new Set(portfolioFilteredRows.map((item) => item.institution))).map((institution) => <span key={institution}>{institution} <strong>{formatMoney(portfolioFilteredRows.filter((item) => item.institution === institution).reduce((sum, item) => sum + Number(item.marketValue), 0))}</strong></span>) : <span>Cadastre seu primeiro investimento para acompanhar por instituição.</span>}</div></div><article className="panel table-panel"><div className="data-table desktop-table"><div className="table-row table-header"><span className="checkbox" /><span>descrição</span><span>categoria</span><span>instituição ↑</span><span>data ↑</span><span className="text-right">valor ↓</span><span /></div>{rows.length ? rows.map((item, index) => <div className={`table-row ${index % 2 ? "selected-row" : ""}`} key={item.id}><span className="checkbox" /><strong>{item.description}</strong><span>{item.category}</span><span>{item.institution}</span><span>{new Date(item.investedAt).toLocaleDateString("pt-BR")}</span><strong className="text-right">{formatMoney(Number(item.marketValue))}</strong><button className="square-btn" onClick={() => onEdit(item)} aria-label={`Editar ${item.description}`}><Pencil className="h-4 w-4" /></button></div>) : <EmptyState icon={TrendingUp} title="Nenhum investimento cadastrado." description="Adicione um investimento para acompanhar seu patrimônio e resultado acumulado." actionLabel="novo investimento" onAction={onAdd} />}</div><div className="mobile-investments">{rows.map((item) => <div className="mobile-investment-row" key={`${item.id}-mobile`}><div><p className="font-semibold text-slate-200">{item.description}</p><p className="mt-1 text-xs text-slate-500">{item.category} · {item.institution} · {new Date(item.investedAt).toLocaleDateString("pt-BR")}</p></div><strong>{formatMoney(Number(item.marketValue))}</strong></div>)}</div></article></div>;
}
...
function FinanceFormModal({ kind, item, profileId, cards = [], onClose, onDelete }: { kind: FinanceFormKind; item?: FinanceFormItem; profileId?: number; cards?: Array<{ id: number; name: string }>; onClose: () => void; onDelete?: (item: FinanceFormItem) => void }) {
  ...
  const finish = (message: string) => { utils.finance.bootstrap.invalidate(); utils.finance.transactions.invalidate(); utils.finance.routine.invalidate(); utils.finance.couple.invalidate(); utils.finance.audit.invalidate(); utils.finance.savingsGoals.invalidate(); toast.success(message); onClose(); };
  ...
  const submit = () => {
    ...
    if (kind === "bill") {
      ...
      const payload = { description: form.description.trim(), dueDate: new Date(`${form.dueDate}T12:00:00`), amount, responsible: form.responsible.trim(), status: form.status as "pending" | "paid" };
      if (item?.id) updateBill.mutate({ profileId, id: Number(item.id), ...payload }, { onSuccess: () => finish("Conta atualizada"), onError: fail }); else createBill.mutate({ profileId, ...payload }, { onSuccess: () => finish("Conta salva"), onError: fail });
      return;
    }
    if (kind === "investment") {
