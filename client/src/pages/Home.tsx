...
function WorkspaceHeader({ active, person, month, onMonthChange, onPersonChange, onAdd, theme, onToggleTheme }: { active: View; person: Person; month: string; onMonthChange: (month: string) => void; onPersonChange: (person: Person) => void; onAdd: () => void; theme: "light" | "dark"; onToggleTheme: () => void }) {
  const pageCopy: Record<View, { title: string; description: string; edit: string }> = {
    overview: { title: "Seu dinheiro, em perspectiva.", description: "Um retrato honesto do seu dinheiro neste mês.", edit: "editar visão" },
    couple: { title: "Resumo geral do casal.", description: "Felipe + Sara em uma única leitura visual.", edit: "editar visão do casal" },
    accounts: { title: "Contas a pagar.", description: "Vencimentos, faturas e pendências em um só lugar.", edit: "editar contas" },
    investments: { title: "Investimentos separados.", description: "Abasteça seus investimentos sem misturar com os lançamentos gerais.", edit: "editar investimentos" },
    cards: { title: "Cartão de crédito.", description: "Acompanhe faturas, compras internas, parcelas e vencimentos.", edit: "editar cartões" },
    trash: { title: "Lixeira.", description: "Recupere um registro ou remova tudo de forma permanente.", edit: "editar lixeira" },
  };
...
function Accounts({ profileId, month = "2026-08" }: { profileId?: number; month?: string }) {
  const { isAuthenticated } = useAuth();
...
function Investments({ profileId, month = "2026-08", person }: { profileId?: number; month?: string; person?: Person }) {
  const { isAuthenticated } = useAuth();
  const { data } = trpc.finance.bootstrap.useQuery({ profileId, month }, { enabled: isAuthenticated && !!profileId });
  const [monthFilter, setMonthFilter] = useState("");
...
function Cards({ profileId, month = "2026-08" }: { profileId?: number; month?: string }) {
  const { isAuthenticated } = useAuth();
  const { data } = trpc.finance.bootstrap.useQuery({ profileId, month }, { enabled: isAuthenticated && !!profileId });
